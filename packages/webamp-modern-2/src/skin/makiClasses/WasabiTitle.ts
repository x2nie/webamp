...
import { num, px, relat } from "../../utils";
...
export default class WasabiTitleBar extends Group {
  ...
  _renderX() {
    this._div.style.left = this._relatx=='1' ? relat(this._padtitleleft + this._x ?? 0) : px(this._padtitleleft + this._x ?? 0);
  ...
  _renderWidth() {
    ...
    this._div.style.width = this._relatw=='1' ? relat(-this._padtitleleft + -this._padtitleright + this._width??0) : px(-this._padtitleright + this.getwidth());
  }
