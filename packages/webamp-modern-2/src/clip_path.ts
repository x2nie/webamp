...
function main() {
  ...
  img2.onload = (ev) => {
    ...
    document.getElementById("top").textContent = edge.top.replace(/px/gi, "").replace(/\,\s/gi, "\n");
    document.getElementById("right").textContent = edge.right.replace(/px/gi, "").replace(/\,\s/gi, "\n");
    document.getElementById("bottom").textContent = edge.bottom.replace(/px/gi, "").replace(/\,\s/gi, "\n");
